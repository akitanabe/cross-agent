---
name: codex-subagent
description: cross-agent から委譲される Codex CLI（codex exec）固有のサブスキル。skill_session_id を Codex の thread_id にマッピングしてセッションを継続し、レビューを実行する。通常はユーザーが直接呼ばず、cross-agent オーケストレーターから呼び出される。
...
cross-agent から渡される `skill_session_id` とコンテキストを受け取り、Codex CLI で
...
- `skill_session_id` → `thread_id` のマッピング（自前管理）
...
| 引数 | 内容 |
|---|---|
| `skill_session_id` | cross-agent が生成したセッション ID |
...
`${CLAUDE_PLUGIN_DATA}/sessions/<skill_session_id>.json` の `agents.codex.thread_id` を
読み書きする。
...
Codex の最終メッセージ（レビュー本文）のパスまたは内容を返す。深掘りループの判断・統合は
