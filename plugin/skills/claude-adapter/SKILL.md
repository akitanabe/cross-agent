...
`round-<N>-<agent_id>-output.md` に保存する。実行後は runner の `complete` で Claude 固有 state と
...
- `round-<N>-<agent_id>-input.md` が返った場合: Claude レビュー本文の生成へ進む
...
`round-<N>-<agent_id>-input.md` を読み、次を確認する。

- 現在 round の `prompt_file` が記載されている
- Claude 固有の `claude_context_file` が記載されている
- 出力保存先として `round-<N>-<agent_id>-output.md` が記載されている
...
`round-<N>-<agent_id>-input.md` に記載された保存先へ、最終レビュー本文だけを書く。

```text
.../artifacts/<review_session_id>/round-<N>-<agent_id>-output.md
```
...
`round-<N>-<agent_id>-response.json` の file path だけを返す。
...
```text
${CLAUDE_PLUGIN_DATA}/sessions/<review_session_id>/agents/<agent_id>.json
${CLAUDE_PLUGIN_DATA}/sessions/<review_session_id>/agents/<agent_id>-context.md
${CLAUDE_PLUGIN_DATA}/artifacts/<review_session_id>/round-<N>-<agent_id>-input.md
${CLAUDE_PLUGIN_DATA}/artifacts/<review_session_id>/round-<N>-<agent_id>-output.md
${CLAUDE_PLUGIN_DATA}/artifacts/<review_session_id>/round-<N>-<agent_id>-diagnostic.md
${CLAUDE_PLUGIN_DATA}/artifacts/<review_session_id>/round-<N>-<agent_id>-response.json
```
...
- `round-<N>-<agent_id>-output.md` にはレビュー本文だけを書く
